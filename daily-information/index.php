...
<head>
	...
	<link rel="stylesheet" type="text/css" href="css/daily-information.css "/>
</head>
<body class="page-body">
		...
		<section class="section-to-print">
			<div class="daily-information page-a4">
				<div class="page-contents page-adjustable">
					...
					?>
					<h1 class="typography-header daily-information-title">일일업무보고</h1>
							...
							<table class="counting">
								<tr>
									<td colspan="5" style="font-weight: 700; position: relative;">
										<p style="color: red;">출동률: <?php echo $width = round((num_go_out($platoon_0_absent, 0) + num_go_out($platoon_1_absent, 1) + num_go_out($platoon_2_absent, 2) + num_go_out($platoon_3_absent, 3)) / (count(get_currently_serving_member(null, $today))) * 100, 1); ?>%</p>
								...
								<tr class="total-row">
									...
									<td contenteditable="true"><?php echo (9 - $agent_total_absent_num - $agent_total_remain_num).' / '.(num_go_out($platoon_0_absent, 0) + num_go_out($platoon_1_absent, 1) + num_go_out($platoon_2_absent, 2) + num_go_out($platoon_3_absent, 3));  ?></td>
								</tr>
								<?php
								for ($i = 0; $i < 4; $i++) {
								?>
								<tr>
									...
									<td contenteditable="true"><?php echo ($platoon_agent_total_num[$i] - $platoon_agent_absent_num[$i] - $platoon_agent_remain_num[$i]).' / '.num_go_out(${"platoon_".$i."_absent"}, $i); ?></td>
								</tr>
								<?php
								}
								?>
								...
								<tr>
									...
									foreach ($agents_working_array as $key => $agent) {
										if ($agent['working_group'] == 6) {
											...
											if ($agent['sub_working_type']) {
												if ($agent['sub_working_type'] == 9) {
													echo '<br>(' . $agent['etc_event_name'] . ')</div>';
												} else {
	...
	<script src="js/daily-information.js"></script>
</body>
</html>
